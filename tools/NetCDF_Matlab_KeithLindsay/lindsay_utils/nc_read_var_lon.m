function field = nc_read_var_lon(filename, varname)
% Read longitude for a netCDF variable
...
% if such a variable exists and it is a valid longitude,
% then we're done
...
for dimnum = 1:length(dimids),
    ...
    for varnum = 0:nvars_file-1,
        ...
        if (strcmp(dimname, varname_tmp))
            if (has_att(fid, varnum, 'units') && is_lon(netcdf.getAtt(fid, varnum, 'units')))
                netcdf.close(fid);
...
disp(['no longitude found for ' varname]);
