function calendar = nc_read_var_time_calendar(filename, varname)
% Read time calendar for a netCDF variable
% nc_read_var_time_calendar(filename, varname)
%
...
for varnum = 0:nvars_file-1,
    ...
    if (strcmp(dimname, varname_tmp))
        if has_att(fid, varnum, 'units')
            ...
            if is_time(units)
                calendar = netcdf.getAtt(fid, varnum, 'calendar');
                netcdf.close(fid);
                return;
            end;
        end;
